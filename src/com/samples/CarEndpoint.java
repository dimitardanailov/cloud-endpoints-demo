...
public class CarEndpoint {
	...
	public CollectionResponse<Car> listCar(
		...
		try {
			...
			Query query = mgr.newQuery(Car.class);
			...
			execute = (List<Car>) query.execute();
	...
	public Car getCar(@Named("id") Long id) {
		...
		Car car = null;
		try {
			car = mgr.getObjectById(Car.class, id);
		} finally {
			mgr.close();
		}
		return car;
	 ...
	 *
	 * @param car the entity to be inserted.
	 ...
	 *
	 * @param car the entity to be updated.
	...
	public void removeCar(@Named("id") Long id) {
		PersistenceManager mgr = getPersistenceManager();
		try {
			Car car = mgr.getObjectById(Car.class, id);
			mgr.deletePersistent(car);
		} finally {
			mgr.close();
		}
	}
	...
	private boolean containsCar(Car car) {
		...
		try {
			mgr.getObjectById(Car.class, car.getId());
